...
import { useUpdatePayment } from '@/hooks/usePurchaseInvoices';
import SupplierPaymentModal from './Supplierpaymentmodal';

// ─── Status badge config (only APPROVED & PARTIALLY_PAID appear here) ────
...
function SummaryCards({ invoices }: { invoices: PurchaseInvoice[] }) {
  const totalDue = invoices.reduce(
    (sum, inv) => sum + (Number(inv.net_amount) - Number(inv.paid_amount)),
    0,
  );
        ...
        {isLoading ? (
          ...
          <div className="p-20 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Toutes les factures approuvées ont été réglées.
            </p>
          </div>
        ) : (
          <>
            <table className="w-full">
              <thead className="bg-gray-50 border-b text-xs text-gray-500 uppercase tracking-wide">
                <tr>
                  <th
                    className={thCls('invoice_number_supplier')}
                    onClick={() => handleSort('invoice_number_supplier')}
                  >
                    N° Facture <SortIcon field="invoice_number_supplier" />
                  </th>
                  <th
                    className={thCls('supplier')}
                    onClick={() => handleSort('supplier')}
                  >
                    Fournisseur <SortIcon field="supplier" />
                  </th>
                  <th
                    className={thCls('invoice_date')}
                    onClick={() => handleSort('invoice_date')}
                  >
                    Date <SortIcon field="invoice_date" />
                  </th>
                  <th
                    className={`${thCls('due_date')} `}
                    onClick={() => handleSort('due_date')}
                  >
                    Échéance <SortIcon field="due_date" />
                  </th>
                  <th
                    className={`${thCls('net_amount')} text-right`}
                  ...
                  return (
                    <tr key={inv.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 text-center">
                        {statusCfg && (
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full font-medium ${statusCfg.classes}`}
                          >
                            {statusCfg.icon}
                            {statusCfg.label}
                          </span>
            ...
            {(data?.total_pages ?? 1) > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t bg-gray-50">
                <p className="text-sm text-gray-500">
                  ...
                  <span className="font-medium">{data?.total}</span> facture(s)
                </p>
...
{paymentInvoice && (
  <SupplierPaymentModal
    ...
    onSuccess={() => {
      ...
      // or you can call queryClient.invalidateQueries if needed
    }}
  />
)}
