// src/console/pages/ConsoleLoginPage.tsx
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.totp_required) {
        navigate('/console/totp-verify', { state: { email } });
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#534AB7] to-[#7C3AED] flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold">Platform Admin Console</CardTitle>
          <CardDescription>
            Sign in to access the NovEntra admin dashboard
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 text-center text-sm text-gray-500">
            <p>Secure platform administration access</p>
          </div>
        </CardContent>
      </Card>
    </div>
