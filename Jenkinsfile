    ...
    stages {
        ...
        stage('🔧 Setup Node.js & pnpm') {
            steps {
                dir('PI-DEV-FRONT') {
                    script {
                        echo "📦 Setting up Node.js ${NODE_VERSION} and pnpm..."
                        sh """
                            node --version
                            npm --version
                            npm install -g pnpm
                ...
                script {
                    ...
                    withCredentials([file(credentialsId: KUBECONFIG_CREDENTIALS_ID, variable: 'KUBECONFIG')]) {
                        sh """
                            ...
                            echo "\n🔌 Services:"
                            kubectl --kubeconfig=\$KUBECONFIG get svc -n ${NAMESPACE} frontend
